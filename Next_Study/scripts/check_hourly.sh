...
set -u
H="root@provider.h100.siamaidol.com"; P=${PORT:-32055}
...
check(){
  N=$((N+1)); echo "===== HOURLY CHECK (final closure) #$N $(date -u +%F_%T) UTC ====="
  local out="" t
  for t in 1 2 3 4; do out=$(ssh $SSHOPT -p $P $H '
    S=/workspace/Quant_Bias/Next_Study/results/final_closure/_logs
    ...
    cd /workspace/Quant_Bias/Next_Study && /opt/conda/bin/python -m next_study.run validate $( [ "$(cat /workspace/MODE_OVERRIDE 2>/dev/null || echo smoke)" = smoke ] && echo --smoke ) --models F-M1 F-M2 F-M3 --partial 2>&1 | grep -E "VALIDATE|FAIL" | head -12
  ...
  echo "$out"
  if echo "$out" | grep -qE "PROC_ALERT|VALIDATE_FINAL_CLOSURE FAIL|FAIL |failed=[1-9]|Connection refused|Permission denied"; then echo ">>> ATTENTION NEEDED"; else echo ">>> all clear"; fi
