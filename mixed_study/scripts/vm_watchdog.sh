...
# Writes a one-line heartbeat to results/v2/_logs/watchdog.log and a JSON
# status file so a remote reader can see the verdict without SSH-ing.
set -uo pipefail
ROOT=/workspace/Quant_Bias; MS=$ROOT/mixed_study
INTERVAL=${WATCHDOG_INTERVAL:-3600}
PIDFILE=/workspace/mixed_watchdog.pid
echo $$ > "$PIDFILE"
...
LOGS=$MS/results/v2/_logs; mkdir -p "$LOGS"
...
check(){
  ...
  ( cd "$ROOT" && git add -A mixed_study/results 2>/dev/null
    if ! git diff --cached --quiet; then
      git commit -q -m "watchdog(mixed): hourly validate + push" 2>/dev/null
